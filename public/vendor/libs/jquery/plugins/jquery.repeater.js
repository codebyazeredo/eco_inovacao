...
(function ($) {
  ...
  $.fn.repeater = function (fig) {
    ...
    $(this).each(function () {
      ...
      $self.on('click', '[data-repeater-copy]', function () {
        const groupName = getGroupName();
        const inputName = $(this).data('repeaterCopy');
        $self.find('[data-repeater-list]').first().find('[name]').each((index, item) => {
          if (item.getAttribute('name') == groupName + '[0]' + '[' + inputName + ']') {
            return;
          }

          if (item.getAttribute('name').includes(inputName)) {
            const jQueryItem = $(item);
            const valor = $(`[name^='${groupName}[0][${inputName}]']`).val();
            jQueryItem.val(valor);
            if (jQueryItem.hasClass('select2-hidden-accessible')) {
              jQueryItem.trigger('change');
            }
          }
        });
      });
